...
int getNumProcessors (void)
{
	static int np = 0;
	if (!np) {
		// Ugly workaround.  Linux's sysconf indirectly calls malloc()
		// (at least on multiprocessors).  So we just read the info 
		// from the proc file ourselves and count the occurrences of 
		// the word "processor".
...
void setCPU (int n) {
	...
	CPU_ZERO(&mask);
	CPU_SET(n, &mask);
	if ((result = pthread_setaffinity_np(thread, sizeof(cpu_set_t), &mask)) != 0) {
		errno = result;
